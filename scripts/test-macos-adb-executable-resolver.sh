...
GB_TMP=$(mktemp -d "${TMPDIR:-/tmp}/galaxybridge-adb-resolver-spec.XXXXXX")
...
xcrun swiftc \
    ...
    "$GB_ROOT/macos/GalaxyBridgeMacTests/ADBExecutableResolverSpec.swift" \
    -o "$GB_TMP/ADBExecutableResolverSpec"

DYLD_LIBRARY_PATH="$GB_TMP" "$GB_TMP/ADBExecutableResolverSpec"
