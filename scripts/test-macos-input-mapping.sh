...
GB_TMP=$(mktemp -d "${TMPDIR:-/tmp}/galaxybridge-input-spec.XXXXXX")
...
xcrun swiftc \
    ...
    "$GB_ROOT/macos/GalaxyBridgeMacTests/DeviceInputMappingSpec.swift" \
    -o "$GB_TMP/DeviceInputMappingSpec"

DYLD_LIBRARY_PATH="$GB_TMP" "$GB_TMP/DeviceInputMappingSpec"
